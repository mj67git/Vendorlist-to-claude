 ...
 * store either way, and this call exists for its status code. The view is held
 * back until the answer arrives — rendering it meanwhile and taking it away a
 * moment later would show the page to somebody who may not open it, which is
 * the whole thing this is here to prevent.
 *
 * What this does NOT do is keep the rows out of the browser. The archive and
 * the directory are different readings of the source list, which this account
 * already holds under `vendor.read` and sees in the category pages; the rows it
 * may not see at all — samples, the blacklist — are dropped by the server
 * before they are sent (`readableVendors`). So the boundary these two
 * permissions draw is which view may be opened, and it is drawn here.
 */
export type ViewAccess = 'checking' | 'allowed' | 'denied';

export function useServerViewAccess(view: string | null, enabled = true, identity?: string | null): ViewAccess {
  ...
  useEffect(() => {
    ...
    // `identity` is in the list so signing in as somebody else asks again
    // rather than carrying the previous account's answer into the new session.
  }, [view, enabled, identity]);
