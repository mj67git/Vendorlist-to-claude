...
say "حساب اولیه:        admin — رمز تصادفی، در لاگ سرور:  docker compose logs pharma-app | grep -A4 \"حساب مدیر سیستم\""
say ""
...
say "  خالی‌کردن داده: ./deploy/reset-data.sh --all   (فقط پیش از شروع استفادهٔ واقعی)"
