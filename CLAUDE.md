...
8a. **ذخیره هرگز صفحه را عوض نمی‌کند.** بعد از یک ثبت موفق، کاربر روی همان صفحه می‌ماند و توست تأیید می‌گیرد؛ اگر جایی برای رفتن هست، **دکمهٔ اقدام روی توست** (`notify(msg, kind, ms, { label, run })`) بگذار نه ناوبری خودکار. تنها استثنا حذف است، چون رکورد دیگر وجود ندارد. `handleAddVendor` قبلاً به صفحهٔ رکورد تازه می‌پرید و همین کارِ ثبت انبوه را عذاب‌آور کرده بود.
