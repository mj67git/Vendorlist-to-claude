...
**باقی‌مانده (تصمیم‌گیری‌نشده):** `/api/health` با دیتابیس خاموش هم `ok` می‌دهد · ورودی نامعتبر روی بعضی endpointها ۵۰۰ می‌دهد نه ۴۰۰ · مسیر API ناموجود ۲۰۰+HTML برمی‌گرداند نه ۴۰۴ · داشبورد برای کاربر بدون `audit.read` یک ۴۰۳ بی‌مصرف می‌زند · `PORT` خوانده نمی‌شود (۳۰۰۰ هاردکد) · سقف `limit` روی audit اعمال نمی‌شود · JWT در localStorage نگهداری می‌شود.
