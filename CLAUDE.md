...
7. **Theme tokens:** برای UI جدید از توکن‌ها استفاده کن (`bg-card`, `text-foreground`, `text-muted-foreground`, `bg-muted`, `border-border`, `bg-background`)، نه رنگ hardcode (`bg-white`, `bg-slate-*`, `text-slate-*`, hex روشن). dark mode با کلاس `.dark` روی `documentElement`؛ سوییچر تم در هدر (`src/hooks/useTheme.ts`).
   - **مقیاس متن در `@theme` تعریف شده و اندازهٔ دلخواه ننویس:** `text-2xs` ۱۲px (کف مطلق) · `text-xs` ۱۳٫۵px (بدنه و جدول) · `text-sm` ۱۵px (فیلد) · `text-xl` ۲۲px (عنوان صفحه). خط فارسی حرف بزرگ ندارد، نقطه‌ها و اعراب در اندازهٔ کوچک به هم می‌چسبند و ارتفاع x نسبی وزیرمتن پایین‌تر از فونت‌های لاتین رابط است — ۱۲px فارسی تقریباً مثل ۱۰٫۵px لاتین خوانده می‌شود. مقادیر با `rem` نوشته شده‌اند تا تنظیم اندازهٔ متن مرورگر/سیستم‌عامل اثر کند؛ px ننویس.
   - **اندازه را در کامپوننت عوض نکن، توکن را عوض کن.** ابزارهای Tailwind v4 به `var(--text-…)` ارجاع می‌دهند، پس یک ویرایش در `index.css` هر ۸۶۰ نقطه را با هم جابه‌جا می‌کند.
   - **چاپ از این مقیاس پیروی نمی‌کند.** `PrintableForms.tsx` از همین توکن‌ها استفاده می‌کند ولی روی کاغذ A4 با صفحه‌بندی تأییدشده؛ بلوک `@media print` در `index.css` مقادیر قدیمی (۱۱/۱۲/۱۴/۲۰px) را برای چاپ برمی‌گرداند. اگر توکنی اضافه کردی که در نمای چاپ مصرف دارد، همان‌جا هم قرارش بده.
   - **`color-scheme` را دست نزن ولی یادت باشد هست:** `:root` روشن و `.dark` تاریک (در `index.css`). نیمی از رابط را مرورگر می‌کشد — علامت چک‌باکس، فهرست بازشدهٔ `<select>`، اسکرول‌بار، تقویم — و روشن/تاریک بودنش را از این می‌گیرد نه از توکن‌های ما. وقتی `normal` بود، چک‌باکس تیک‌نخورده در دارک مربع سفید بود.
   - **tooltip در دارک مرز دارد (`dark:border-border`)**، چون `dark:bg-muted` دقیقاً هم‌رنگ `--card` است و روی هر کارتی بدون مرز گم می‌شد.
   ...
   - **دکمه و فیلد را دستی نساز.** `<button>` خام با کلاس دستی ننویس — از `ui/button.tsx` استفاده کن (واریانت‌ها: `default, destructive, outline, secondary, ghost, link, success` · اندازه‌ها: `default, sm, lg, icon, icon-sm, icon-xs`). ورودی متنی از `ui/input.tsx` می‌آید. استثناها عمدی‌اند و **دکمه به معنای سیستم طراحی نیستند**: سرستون مرتب‌سازی، تب، چیپ فیلتر، ردیف منو یا گزینه، خانهٔ تقویم، آیتم سایدبار، لینک متنی breadcrumb، آیکن درون‌خطی داخل نوار خطا یا فیلد (مثل چشمِ رمز) و نمای چاپ.
   - **`<select>` بومی می‌ماند ولی ظاهرش از `inputBaseClass` می‌آید** (`cn(inputBaseClass, …)`). پریمیتیو رادیکسیِ select عمداً در پروژه نیست: با `onValueChange` کار می‌کند نه رویداد change، پس بردن ~۳۷ کشوی موجود روی آن بازنویسی رفتار بود نه تغییر ظاهر — فایلش بی‌مصرف مانده بود و حذف شد. رنگ حلقهٔ focus تازه اختراع نکن — قبلاً پنج‌تا رنگ مختلف در فرم‌ها بود.
   - `Textarea` هم کلاس پایه‌اش را از `inputBaseClass` می‌گیرد (فقط قواعد ارتفاع فرق دارد)، پس فیلد متن و کادر توضیح هیچ‌وقت دو حلقهٔ focus متفاوت ندارند.
   - **ردیف‌های داخل بدنهٔ جدول کامپوننت مشترک دارند:** `ui/sort-header.tsx` (سرستون مرتب‌سازی)، `ui/table-empty-row.tsx` (خالی و خطا) و `ui/table-skeleton-rows.tsx` (بارگذاری). دست‌ساز ننویس — همین چهار کپیِ واگرا بود که باعث شد یک جدول `aria-hidden` روی ردیف‌های ساختگی نداشته باشد و صفحه‌خوان دادهٔ جعلی بخواند.
8. **هر لایهٔ روی صفحه از `FormModal` می‌آید (`src/components/FormModal.tsx`).** backdrop، Esc، کلیک بیرون، قفل اسکرول، focus trap و انیمیشن **متقارن** ورود/خروج آنجاست — مودال دستی نساز. سه اندازه: `sm` تأییدیه، `md` فرم تک‌منظوره، `lg` فرم کامل رکورد.
   ...
   - تأییدیه‌های مخرب: `closeOnBackdrop={false}` **و** `role="alertdialog"` بگذار (Esc همچنان لغو می‌کند).
   - **فرمی که ورودی کاربر دارد باید `unsavedChanges` بدهد** (و `unsavedLabel` برای اینکه بگوید چه چیزی از بین می‌رود). Esc و کلیک بیرون کادر آنگاه تأییدیه می‌گیرند نه بستن بی‌صدا. سیگنال را با `useDirtySnapshot` بساز — مقایسه با snapshotِ باز شدن، نه «باز بودن فرم»، وگرنه هشدار نویز می‌شود و کاربر یاد می‌گیرد بی‌خواندن ردش کند. فرم درون‌صفحه‌ای (نه مودال) به‌جای این، با `onDirtyChange` به صفحه گزارش می‌دهد و صفحه `registerNavGuard` را ثبت می‌کند (قاعدهٔ ۱۰).
...
8a. **ذخیره صفحه را عوض نمی‌کند، مگر ثبت یک رکورد تازه که مقصد بعدیِ کار خودش است.**
   - **قاعدهٔ عمومی:** بعد از ذخیرهٔ یک ویرایش، کاربر روی همان صفحه می‌ماند و توست تأیید می‌گیرد؛ اگر جایی برای رفتن هست، **دکمهٔ اقدام روی توست** (`notify(msg, kind, ms, { label, run })`) بگذار نه ناوبری خودکار.
   - **استثنای ثبت سورس تازه (خواستهٔ صریح کاربر، ۱۴۰۵/۰۶/۱۶):** «ثبت سورس» به صفحهٔ همان رکورد می‌رود، چون کار آنجا ادامه دارد — امتیازدهی دپارتمان‌ها، ارزیابی ریسک، بقیهٔ ارزیابی. چیزی که این پرش را قبلاً آزاردهنده کرده بود ثبت انبوه بود، و **دکمهٔ «ذخیره و ثبت بعدی» همان را حل می‌کند**؛ حذف پرش راه‌حلش نیست.
   - **دو دکمه، یک منطق ذخیره.** فقط اقدامِ پس از ذخیره فرق می‌کند. منطق ذخیره را کپی نکن.
   - **ذخیره باید await شود و در صورت خطا نه ناوبری کند نه فرم را پاک کند.** `handleAddVendor` یک `Promise<Vendor | null>` برمی‌گرداند: رکورد ذخیره‌شده در موفقیت، و `null` بعد از rollback و نمایش خطا. عمداً reject نمی‌کند تا فراخوانی‌هایی که نتیجه برایشان مهم نیست unhandled rejection نسازند. تا وقتی پاسخ سرور نیامده، محافظ تغییرات ذخیره‌نشده مسلح می‌ماند و فیلدها پر.
   - فرم‌های ثبت **دکمهٔ «ذخیره و ثبت بعدی»** دارند (فقط در حالت جدید): ذخیره + خالی‌کردن فرم در جا + ماندن روی صفحه، به‌همراه شمارندهٔ نشست. اگر فرم ثبتی اضافه کردی، همین را هم بده.
...
10. **مدل ناوبری (`src/App.tsx`) — رعایت کن:** منبع حقیقتِ ناوبری، استکِ `viewHistory` است (persist در localStorage با کلید `app_viewHistory`).
   ...
   - **رکورد ذخیره‌شده را روی «آخرین ورودی» ننویس؛ روی ورودی‌هایی بنویس که همان سورس را نشان می‌دهند** (`refreshVendorEverywhere` در `navStack.ts`). ذخیره صفی با `await` است و چند صد میلی‌ثانیه طول می‌کشد؛ اگر کاربر در آن فاصله صفحه را عوض کند، نوشتن روی آخرین ورودی سورس را روی صفحهٔ اصلی می‌نشاند و صفحه دوباره به سورس برمی‌گردد (`#/vendor/<id>` بدون دسته‌بندی = همین باگ). به همین دلیل هر callbackی که بعد از پاسخ سرور اجرا می‌شود باید اول بپرسد کاربر هنوز همان‌جاست یا نه — از جمله `onSaved` فرم سورس.
...
11a. **کار هم‌زمان چند کاربر سه لایه دارد؛ هر سه را رعایت کن (`src/server/http/recordLock.ts`).**
   - **قفل per-record:** هر مسیر نوشتنیِ دارای `:id` باید `serializeWrites("<scope>")` را در زنجیرهٔ middleware داشته باشد (`serializeVendorWrites` همان است با scope سورس). رکوردی که کلید ترکیبی دارد — مثل `source_selections` — قفل را دستی با `lockRecordWrite(scope, key)` می‌گیرد و در `finally` آزاد می‌کند. قفل درون‌پروسه‌ای است، پس روی چند نسخه فقط گارد timestamp می‌ماند.
   - **گارد نسخه (۴۰۹):** هندلر بعد از خواندن رکورد `staleCopy(req, current)` را صدا می‌زند و در صورت مغایرت با `STALE_COPY_MESSAGE` پاسخ **۴۰۹** می‌دهد. کلاینت همان `updatedAt`ی را که خوانده در `expectedUpdatedAt` پس می‌فرستد. **نبودِ این کلید یعنی «ادعایی نشده»** و رفتار قدیمی حفظ می‌شود (کلاینت قدیمی، اسکریپت، مسیر ساخت). مقدار خراب هم نادیده گرفته می‌شود تا ساعتِ خرابِ یک مرورگر رکورد را غیرقابل‌نوشتن نکند.
   - **بعد از ذخیرهٔ موفق، مهر زمانی محلی را تازه کن.** پاسخ هر endpoint رکورد نوشته‌شده را برمی‌گرداند؛ اگر `updatedAt` تازه را در state ننشانی، ویرایش بعدیِ همان نشست با ادعای کهنه ۴۰۹ می‌گیرد — تعارضی که طرف مقابلش هیچ‌کس نیست. در ذخیرهٔ چندمرحله‌ای (صف `syncQueue` سورس) مهر زمانی باید از پاسخ هر مرحله به مرحلهٔ بعد **منتقل** شود.
   - **همگام‌سازی پس‌زمینه:** `GET /api/vendors/changes?since=<ISO>` فقط id، مهر زمانی و تعداد را می‌دهد. کلاینت هر ۳۰ ثانیه (و هنگام برگشتن تب) می‌پرسد؛ `since` همیشه `serverTime` پاسخ قبلی است نه ساعت مرورگر. **اگر فرمی dirty باشد چیزی refetch نمی‌شود** — نوار پیشنهاد بالای صفحه می‌آید و تصمیم با کاربر است؛ در غیر این صورت داده بی‌صدا جایگزین می‌شود و **صفحه هیچ‌وقت نمی‌پرد**. نوشته‌های خودِ همین نشست با id فیلتر می‌شوند. حذف با تغییر `total` فهمیده می‌شود چون ردیف حذف‌شده مهر زمانی ندارد.
...
14. **دسترسی سمت‌سرور اجباری است، نه سمت‌کلاینت.** گیت‌های `role === 'admin'` در فرانت فقط برای UX‌اند (چون `currentUser` از localStorage می‌آید و قابل ویرایش است). هر endpoint حساس باید `requireAuth` + `requireRole('admin')` داشته باشد (`server.ts`).
   ...
   - **جدول سیاست واحد `src/utils/permissions.ts`** — هم `server.ts` و هم کامپوننت‌ها از همین می‌خوانند. **شرط نقش دستی ننویس**؛ از `can(role, permission)` و `canScoreDepartment(role, dept)` استفاده کن، وگرنه UI و سرور دوباره واگرا می‌شوند (همان چیزی که باعث شد دکمه‌ها مخفی باشند ولی endpoint اجازه بدهد).
     - **خواندن هم مجوز است:** `vendor.read` / `material.read` / `partner.read` (+ `partner.files` فقط برای دانلود مدارک SOP) روی ۹ مسیر GET اعمال می‌شوند، پس ستون «مشاهده» در فرم چک‌باکس واقعی است و «فقط R روی شرکا» قابل بیان است. الگوی هر نقش کاری هر سه خواندن را دارد، پس کسی چیزی از دست نداد.
     - **فهرست ذخیره‌شده دقیقاً همان چیزی است که نوشته شده.** قبلاً هر فهرستِ بدون `read` «قدیمی» فرض می‌شد و خواندن‌ها موقع خواندن به آن اضافه می‌شد؛ آن حدس نمی‌توانست ردیف قدیمی را از محدودسازی عمدی تشخیص دهد، پس حسابی که فقط امتیازدهی داشت بی‌صدا همهٔ خواندن‌ها را پس می‌گرفت و فرم انگار ذخیره نشده بود. مهاجرت `20260903120000` همان ردیف‌های قدیمی را یک‌بار در دیتابیس اصلاح می‌کند و از آن پس فهرست عیناً اجرا می‌شود. نام‌های بازنشسته همچنان با `expandStored` باز می‌شوند.
     - خواندن و نوشتن در فرم به هم قفل‌اند: تیک هر عملیات، مشاهده را روشن می‌کند و برداشتن مشاهده کل ردیف را خاموش.
     - اختیاری که در قالب CRUD نمی‌گنجد در `extras` همان ماژول تعریف می‌شود (نه ستون تازه): چک‌باکس زیر نام ماژول می‌آید و حرفش به بج خلاصه اضافه می‌شود. `partner.files` اولین نمونه است.
     - ماتریس: `admin` همه · `commercial` = خواندن‌ها + `partner.files` + `vendor.create/edit` + `partner.create/edit/delete` + `score.commercial` · `qa` = خواندن‌ها + `partner.files` + `vendor.analysis` + `material.create/edit/delete` + `score.qa` · `planning`/`finance` = خواندن‌ها + `score.<خودشان>` (بدون `partner.files`) · `lab` هیچ (در enum مانده چون حذفش مهاجرت می‌خواهد).
     ...
     - **ماتریس دانه‌ریز (۱۴۰۵/۰۶/۱۷، ۳۴ مجوز):** واحد سیاست «ماژول + عملیات + تصمیم کاری» است، نه ماژول. پنج مجوز قبلی بیش از یک عملیات حمل می‌کردند و حالا تفکیک شده‌اند: `vendor.edit`→`vendor.decide` (رد صلاحیت سورس) · `vendor.analysis`→`sample.decide` (تصمیم کیفی نمونه) · `partner.edit`→`partner.evaluate` + `partner.status` · `users.manage`→`users.read` + `users.permissions` + `users.password` · `vendor.read`→`archive.read` + `supplier-audit.read` + `sample.read` + `blacklist.read`.
     - **`IMPLIED_PERMISSIONS` یعنی تفکیک هیچ‌وقت دسترسی نمی‌گیرد — ولی فقط یک‌بار و در دیتابیس.** مهاجرت `20260908100000` باز شدن را در ردیف‌های ذخیره‌شده می‌نویسد و کد نام **زنده** را موقع خواندن باز نمی‌کند؛ وگرنه تیک‌زدن «مدیریت کاربران» بدون «تعیین سطح دسترسی» غیرممکن می‌شد (مقدار پاک‌شده برمی‌گشت). نام‌های **بازنشسته** همچنان روی خواندن باز می‌شوند، چون هیچ مهاجرتی آن ردیف‌ها را عوض نکرده. هر تفکیک تازه باید هر دو نیمه را داشته باشد: نگاشت + مهاجرت.
     - **تصمیم کاری با middleware روی مسیر بیان نمی‌شود.** `PATCH /api/vendors/:id/profile` و `PUT /api/business-partners/:id` کل رکورد را جایگزین می‌کنند، پس تصمیم داخل یک ویرایش عادی سفر می‌کند. الگو: `requireAnyPermission(...)` روی مسیر + مقایسهٔ payload با رکورد ذخیره‌شده در هندلر (`src/utils/decisionGuards.ts`: `forbiddenVerdictChange`، `forbiddenPartnerDecisions` — همان الگوی `forbiddenScoreChanges`). یک مجوز روی مسیر همان چیزی بود که تصمیم کیفی نمونه را پاسخگوی `vendor.edit` کرده بود.
     - **چهار خواندن تازه، دو جنس:** «نمونه» و «لیست سیاه» دستهٔ ردیف‌اند و سرور با `readableVendors` ردیف‌ها را فیلتر می‌کند (روی وضعیت مشتق، نه ستون خام)؛ حسابِ محدود در دیتابیس صفحه‌بندی نمی‌شود چون «مردود» ستون ندارد، پس پنجره بعد از فیلتر گرفته می‌شود و `changes` هم با همان فهرست شمرده می‌شود. «آرشیو» و «بررسی یکپارچه» همان ردیف‌ها را می‌خوانند، پس گیتشان روی **نما**ست: `GET /api/vendors?view=…`.
     - `archive.read` یک‌بار **حذف شده بود** چون هیچ endpointای اعمالش نمی‌کرد. حالا برگشته، ولی به این شرط که سرور ردیف‌ها و نما را بر اساسش تصمیم بگیرد. **مجوزی که سرور اعمال نمی‌کند نساز**؛ در فرم به‌جای چک‌باکس مرده، خانهٔ قفل‌شده با دلیل بگذار (`PERMISSION_MODULES` + `LOCKED_REASONS`).
     - **`VIEW_PERMISSIONS` تنها جدول «کدام صفحه چه مجوزی می‌خواهد» است** — سایدبار، پالت فرمان (⌘K)، خود صفحه و سرور (`GET /api/vendors?view=…` برای `SOURCE_LIST_VIEWS`) از همین می‌خوانند. صفحهٔ تازه را همان‌جا ثبت کن، وگرنه دوباره جایی می‌ماند که ورودی را پنهان می‌کند و جای دیگری آن را تحویل می‌دهد.
     - **آرشیو و بررسی یکپارچه از استور مشترک نمی‌خوانند.** هرکدام ردیف‌هایش را با `useGatedVendorList` از `GET /api/vendors?view=…` می‌گیرد، پس داده حاصل درخواستی است که سرور می‌توانست ردش کند نه یک شرط در مرورگر (دستکاری `currentUser` در localStorage سایدبار را باز می‌کند ولی نما صفر ردیف می‌دهد). فقط **۴۰۳** نما را می‌بندد؛ شکست دیگر پیام خطا و «تلاش دوباره» می‌گیرد — آرشیوِ خالی شبیه رجیستر خالی است. `dataRevision` در `App.tsx` این کپی را با همگام‌سازی پس‌زمینه هم‌گام نگه می‌دارد (قاعدهٔ ۱۱a). تمیزکاری ورودی مشترک است: `src/utils/vendorNormalize.ts`.
     - **توضیح هر ردیف پیش‌فرض پشت ⓘ است (`ui/info-hint.tsx`) و کلید «نمایش توضیحات» همه را باز می‌کند** (ترجیح در `localStorage`). توضیح تازه را **یک جملهٔ زیر ۱۰۰ کاراکتر** بنویس — تست «توضیح‌ها کوتاه می‌مانند» همین را می‌پاید؛ استدلال بلند جایش کامنت کد و همین فایل است. قبلاً ۲۷۳۴ کاراکتر نثر باعث شده بود روی ۱۳۶۶×۷۶۸ فقط ۲ ردیف از ۱۳ دیده شود.
     - **فرم دسترسی = ماتریس ۱۳ ردیفی (`PERMISSION_MODULES`).** یک مجوز می‌تواند در دو ردیف دیده شود (ثبت/ویرایش/حذف نمونه همان مجوزهای سورس است)؛ **اولین ردیف مالک است و بقیه آینه‌اند** (`permissionOwner` / `ownedModulePermissions`). آینه چک‌باکس زنده ندارد، و خاموش‌کردن «مشاهده» فقط مجوزهای مالکِ همان ردیف را پاک می‌کند — وگرنه بستن دستهٔ نمونه، ثبت سورس را هم می‌گرفت. مجوز تازه باید در یک ردیف قابل تنظیم باشد؛ تست «هر مجوز در فرم قابل تنظیم است» همین را می‌پاید.
     ...
     - **نقش فقط الگوست.** `users.permissions` (JSON) استثنای پر‌کاربر است: **خالی = از نقش پیروی کن** (پس «هیچ دسترسی» با فهرست خالی بیان نمی‌شود — حساب را غیرفعال کن؛ دیالوگ همین را هشدار می‌دهد) (وضعیت همهٔ کاربران قدیمی، پس مهاجرت لازم نبود)، پرشده = همان فهرست **جایگزین** الگو می‌شود. `can(user, perm)` کاربر می‌گیرد نه رشتهٔ نقش.
...
16. **واژگان Audit یک‌جا تعریف می‌شود (`src/utils/auditTaxonomy.ts`) و نوشتن هم یک راه دارد (`src/utils/auditEvents.ts`).**
   - **`AuditEvent` واژگان بسته است (۳۹ رویداد).** هر نام، ماژول و اکشن و شدت و جملهٔ فارسی‌اش را خودش تعیین می‌کند؛ رکورد ممیزی دستی نساز و جملهٔ فارسی در call site ننویس — `recordEvent(req, { event, entity, changes | facts, reason })` را صدا بزن. رویداد تازه = یک ورودی در `AUDIT_EVENTS` + برچسب ماژول/اکشنش در همان فایل.
   - **چهار دروازهٔ `buildAuditRecord` قابل دور زدن نیستند:** فیلد ممنوع (رمز، توکن، base64) حذف می‌شود · فقط تفاوت‌ها می‌مانند · **رویداد بی‌اثر ردیف نمی‌سازد** (مگر `alwaysRecord`) · سقف ۲KB با نشانهٔ صریح. `beforeData` همیشه `null` است: **حذف هیچ‌وقت dump نمی‌کند**، هویت رکورد در ستون‌های خودش است.
   - **`audit_log` append-only است و ستون `event` دارد.** تریگر `audit_log_no_update_delete` هر `UPDATE`/`DELETE` ردیفی را رد می‌کند (مهاجرت `20260909100000`) — کوئری اصلاحی روی سابقهٔ ممیزی ننویس، جواب نمی‌گیرد. فقط `TRUNCATE` باز است (تریگر ردیفی رویش شلیک نمی‌کند) و تنها مصرفش هارنس تست و `prisma/seed.ts` است. ستون `event` نام رویداد از `AUDIT_EVENTS` را نگه می‌دارد و nullable است چون فراخوانی‌های هنوز-مهاجرت‌نشده آن را نمی‌فرستند؛ با `eventType` قدیمی اشتباه نگیر (آن یکی ستونِ بازنشسته است و فیلتری بر آن ساخته نمی‌شود).
   - `diffFields` هیچ‌وقت `updatedAt`/`createdAt`/`id` را گزارش نمی‌کند. ستون‌های `module`/`action`/`severity` در `audit_log` متن آزادند، پس فیلتر فقط وقتی کار می‌کند که مقدارِ ارسالی **دقیقاً** همان چیزی باشد که `server.ts` نوشته است. فهرست دستی در فرم فیلتر ننویس — از `AUDIT_MODULE_LABELS` / `AUDIT_ACTION_LABELS` / `AUDIT_EVENT_GROUPS` استفاده کن (همان الگوی `permissions.ts`: هم سرور هم کلاینت از یک فایل می‌خوانند).
...
## وضعیت امنیتی (بعد از تست نهایی پیش از تحویل)
**رفع‌شده:** کلید ثابت JWT حذف شد و در production نبودش پروسه را متوقف می‌کند · قفل per-vendor جلوی گم‌شدن نوشتن هم‌زمان را می‌گیرد · محدودکنندهٔ نرخ ورود (۸ به‌ازای کاربر / ۶۰ به‌ازای IP در ۱۵ دقیقه) · PBKDF2 با ۲۱۰٬۰۰۰ تکرار و ارتقای خودکار هش‌های قدیمی هنگام ورود · secretها از `docker-compose.yml` به `.env` رفتند · هدرهای امنیتی (CSP و…) بدون افزودن وابستگی.

**رفع‌شده (دور دوم):** `/api/health` واقعاً دیتابیس را چک می‌کند و ۵۰۳ می‌دهد · ورودی نامعتبر ۴۰۰ می‌گیرد و متن خطای داخلی لو نمی‌رود · مسیر API ناموجود ۴۰۴ JSON · داشبورد بدون `audit.read` درخواست نمی‌زند · `PORT` خوانده می‌شود · `limit`/`page` در audit clamp شده‌اند.

**باقی‌مانده (پذیرفته‌شده):** JWT در localStorage نگهداری می‌شود (نه کوکی HttpOnly) — تغییرش بازنویسی جریان احراز هویت است.

**قاعده‌های تازه‌ای که باید رعایت شوند:**
- هر endpoint نوشتنیِ تازه روی `/api/vendors/:id/*` باید `serializeVendorWrites` را در زنجیرهٔ middleware داشته باشد، وگرنه دوباره در معرض همان مسابقهٔ خواندن-تغییر-نوشتن است.
- هیچ مقدار پیش‌فرضی برای secret در کد نگذار؛ الگو `resolveJwtSecret()` است: در production خطا و توقف، در توسعه مقدار تصادفی.
