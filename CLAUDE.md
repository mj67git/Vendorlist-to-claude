...
7. **Theme tokens:** برای UI جدید از توکن‌ها استفاده کن (`bg-card`, `text-foreground`, `text-muted-foreground`, `bg-muted`, `border-border`, `bg-background`)، نه رنگ hardcode (`bg-white`, `bg-slate-*`, `text-slate-*`, hex روشن). dark mode با کلاس `.dark` روی `documentElement`؛ سوییچر تم در هدر (`src/hooks/useTheme.ts`).
   - **کف اندازهٔ متن `text-2xs` (۱۱px) است و مقدار دلخواه ننویس.** خط فارسی حرف بزرگ ندارد، نقطه‌ها و اعراب در اندازهٔ کوچک به هم می‌چسبند و ارتفاع x نسبی وزیرمتن پایین‌تر از فونت‌های لاتین رابط است — ۱۲px فارسی تقریباً مثل ۱۰٫۵px لاتین خوانده می‌شود. قبلاً ۴۹۹ نقطه با `text-[9px]`/`[10px]`/`[11px]` زیر این کف بودند، انگار فقط «کوچک»‌اند در حالی که ناخوانا بودند. توکن در `@theme` تعریف شده (`--text-2xs`) و line-height سخاوتمندش عمدی است. `text-xs` همان ۱۲px می‌ماند؛ عوض‌کردنش ۵۵۸ نقطه را یکجا جابه‌جا می‌کند. تنها استثنا `PrintableForms.tsx` است — تایپوگرافی چاپ قواعد فیزیکی خودش را دارد.
