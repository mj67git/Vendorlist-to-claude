...
11d. **یک رجیستر، یک شرط عضویت (`isInCategoryRegister` در `vendorState.ts`).** صفحهٔ دسته‌بندی، شمارندهٔ سایدبار و خروجی اکسل همگی از همین می‌خوانند. سه کپی دستی داشتیم و دوتایشان سورس مردود را حذف می‌کردند و سومی نه — «خارجی» روی صفحه ۱۰۵ ردیف و در اکسل ۱۴۰ ردیف می‌داد. تست مردودی همیشه `isVendorRejected` است (قاعدهٔ ۱۱)، نه مقایسهٔ دستی با `status`/`grade`.
